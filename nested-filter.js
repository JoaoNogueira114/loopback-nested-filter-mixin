...
module.exports = function(Model, options) {
  Model.beforeRemote('find', function event(ctx, options, next) {
    ...
    if (ctx.methodString.split('.')[1] == 'count') {
      /*
       * Sometimes loopback screws up requests and parses requests with
       * methodString = "CuttingProgram.count"
       * but
       * method.name = "find"
       * which forwards count requets to find hook
       */
      beforeRemoteCount(ctx, options, next);
    } else {
      beforeRemoteFind(ctx, options, next)
    }
  });
  
  Model.beforeRemote('count', function event(ctx, options, next) {
    /* Allows nested filters to be applied - forwards as find method*/
    beforeRemoteCount(ctx, options, next);
  });
  
  Model.afterRemote('find', function event(ctx, options, next) {
    afterRemoteFind(ctx, options, next);
  });
};

// Logic
function beforeRemoteFind(ctx, options, next) {
...
function beforeRemoteCount(ctx, options, next) {
  if (ctx.args.filter && ctx.args.filter.excludeIfEmpty === true) {
    ctx.args.filter.excludeIfEmpty = generateExcludeArray(ctx.args.filter.include);
  }
  if (!ctx.args.where) {
...
function afterRemoteFind(ctx, options, next) {
